...
from optim import BertAdam as AdamW, WarmupLinearSchedule
...
def main():
    ...
    cache_dir = args.cache_dir if args.cache_dir else os.path.join(str(PYTORCH_PRETRAINED_ROBERTA_CACHE), 'distributed_{}'.format(args.local_rank))
    model = RobertaForSequenceClassification.from_pretrained(args.roberta_model,
              cache_dir=cache_dir,
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                output = model(input_ids=input_ids,attention_mask=input_mask,labels=None)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = RobertaForSequenceClassification.from_pretrained(args.roberta_model, num_labels=num_labels)
    model.to(device)
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        cm = metrics.confusion_matrix(all_label_ids, preds)
        disp = ConfusionMatrixDisplay(confusion_matrix=cm)
        disp.plot()
        plt.show()
        plt.savefig("confusionmatrix_relation_prediction.jpg")
